...
const currencyName = config.init("currencyName", "金币");
const eco = (() => {
    switch (currencyType) {
        case "llmoney":
            return {
...
mc.regPlayerCmd(command, "打开物品集市", main);
...
function itemUpload(pl) {
    ...
    pl.sendForm(fm, (pl, args) => {
        ...
        if (item.count < args[3] && pl.isOP() == false) {
        ...
        if (item.count == args[3]) item.setNull();
            ...
            if (pl.isOP() == false) item.setNbt(itemNBT.setByte("Count", Number(item.count - args[3])));
        pl.refreshItems();
