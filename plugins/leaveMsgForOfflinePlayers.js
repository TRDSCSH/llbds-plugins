    ...
    if (!playerMsgData) {
        ...
        if (msgCount > 0) {
            ...
            msg.forEach((item) => {
                ...
                const msg = `§e${data.xuid2name(sender)} §r在 §a${time} §r留言：\n${content}`;
                pl.sendText(msg);
                // 更新玩家的已发送列表
                ...
                logToFile(logFilePath, `[${getTime()}] ${playerName} 查看了 ${data.xuid2name(sender)} 的留言：${content}`);
            });
            // 清空玩家的留言
            playerMsgData.msg = [];
            db.set(playerXuid, playerMsgData);
        ...
        if (slider == 0) {
            ...
            if (playerMsgData) {
                ...
                if (msg.length > 0) {
                    ...
                    if (msgIndex != -1) {
                        if (hasMsgFromPl) {
                            EditMsgGUI(pl, playerXuid, playerXuidArr, hasMsgFromPl, `§6你已经给 ${data.xuid2name(playerXuid)} 留言了， 你可以继续编辑留言内容`, msgArr[msgIndex].content, page);
                        } else {
...
function EditMsgGUI(pl, receiverXuid, playerXuidArr, hasMsgFromPl, hint, content, page) {
    ...
    const fm = mc.newCustomForm()
        .setTitle("离线留言 - 编辑留言")
        .addLabel(`给 §2${data.xuid2name(receiverXuid)} §r留言`)
        .addLabel(hint || "")
    ...
    pl.sendForm(fm, (pl, fmdata) => {
        ...
        if (msg.length < minMsgLength) {
            if (hasMsgFromPl) {
                ...
                logToFile(logFilePath, `[${getTime()}] ${pl.realName} 把给 ${data.xuid2name(receiverXuid)} 留言清除了`);
            } else {
        ...
        if (playerMsgData) {
            ...
            logToFile(logFilePath, `[${getTime()}] ${pl.realName} 给 ${data.xuid2name(receiverXuid)} 留言：${msg}`);
...
//                 pl.tell("§6已舍弃写给 " + data.xuid2name(receiverXuid) + " 的留言:\n" + msg);
//             }
//         }
//     });
// }
