...
-- 导入远程函数 -- Edited
local xuid2name = ll.imports("PLINFO", "xuid2name");
local name2xuid = ll.imports("PLINFO", "name2xuid");

function getAllPlayerInfo()
    local dataPath = "plugins/PlayerInfo/data.json"
    local dataFile = JsonConfigFile(dataPath)

    local players = dataFile:get("players")
    local playerInfo = {}
    ...
    for i, player in ipairs(players) do
        ...
        table.insert(playerInfo, playerData)
    end
...
Land = {
	...
	Storage = {
		...
		Create = function(xuid,AABB,nickname) -- Edited 5
			-- --- Edited 5
			-- --- Set Nickname
			...
			--- Set Permission.
			if xuid == 'public' then
				local truePermissions = {"use_grindstone","use_door","use_trapdoor","use_daylight_detector","allow_attack_player","allow_attack_mobs","use_bed","use_composter","use_beacon","use_dispenser","use_anvil","use_lectern","use_pressure_plate","use_smithing_table","allow_pickupitem","use_barrel","allow_attack_animal","use_bucket","use_loom","allow_throw_potion","use_lever","use_cauldron","use_enchanting_table","allow_shoot","use_fence_gate","use_stonecutter","use_brewing_stand","use_furnace","use_crafting_table","allow_dropitem","use_campfire","use_bell","use_button","use_jukebox","use_smoker","eat","allow_ride_trans","allow_ride_entity","allow_open_chest","use_noteblock","use_blast_furnace","use_fishing_hook","use_shulker_box","use_hopper","use_dropper","use_cartography_table","use_armor_stand"}
				local falsePermissions = {"allow_destroy","allow_entity_destroy","use_respawn_anchor","use_item_frame","allow_place","use_firegen"}
				for _,item in pairs(truePermissions) do
					DataStorage.Land.Raw[landId].permissions[item] = true
				end
				for _,item in pairs(falsePermissions) do
					DataStorage.Land.Raw[landId].permissions[item] = false
				end

				DataStorage.Land.Raw[landId].settings.nickname = nickname -- Edited 5

				DataStorage.Save({0,1,0})
				-- log('Land '..landId..' created by public. Permissions set to default.')
			end
			--- Edited 4 End
	...
	API = {
		RunExport = function()
			...
			for apiName,func in pairs(Land.API.Exported) do -- Edited
				if not ll.export(func,'ILAPI', apiName) then
					ERROR('There was a problem exporting the API, export is failed!')
					return false
				end
			end
		...
		Exported = {
			...
			['CreateLand'] = function(xuid,posA,posB,dimid,nickname)
				...
				local rtn = Land.Storage.Create(xuid,Cube.Create(posA,posB,dimid),nickname) -- Edited 5
				rtn = rtn or -1
				return rtn
			end,
				...
				function(pl,ids)
					...
					ItemSelector.Create(pl,{
						...
						callback = function (selected)
							...
							local targetXuid = name2xuid(selected[1])
	...
	Timer = {
		LandSign = function()
			for xuid,res in pairs(MEM) do
				...
				local ownerId = '?'
				if ownerXuid then
					ownerId = xuid2name(ownerXuid) or '?'
					if ownerId == '' then -- Edited 2
						ownerId = '未知领地' -- Edited 2
					elseif ownerId == 'PUBLIC' then
						ownerId = '公共领地'
					else
						ownerId = ownerId .. ' 的领地' -- Edited 2
					end
				end
				...
				if Land.Util.CheckPerm(landId,xuid) then
					...
					SendTitle(player,
					-- _Tr('sign.listener.visitortitle'),  -- Edited 1
					_Tr('sign.listener.ownertitle','<a>',Land.Options.Nickname.get(landId,2)),  -- Edited 1
			...
			for xuid,res in pairs(MEM) do
				...
				if Land.Util.CheckPerm(landId,xuid) then
					...
					if landcfg.signtother then
						-- player:sendText(_Tr('title.landsign.visitorbuttom','<a>',ownerId),4) -- Edited 3
						player:sendText(_Tr('title.landsign.ownenrbuttom','<a>',Land.Options.Nickname.get(landId,3)),4) -- Edited 3
					end
				end
...
function RegisterCommands()
	...
	mc.regConsoleCmd('iland',_Tr('command.console.land'),function(args)
		if #args~=0 then
	...
	mc.regConsoleCmd('iland op',_Tr('command.console.land_op'),function(args)
		local name = table.concat(args,' ')
		local xuid = name2xuid(name)
	...
	mc.regConsoleCmd('iland deop',_Tr('command.console.land_deop'),function(args)
		local name = table.concat(args,' ')
		local xuid = name2xuid(name)
	...
	mc.regConsoleCmd('iland update',_Tr('command.console.land_update'),function(args)
	...
	mc.regConsoleCmd('iland language',_Tr('command.console.land_language'),function(args)
		INFO('I18N',_Tr('console.languages.sign','<a>',cfg.plugin.language,'<b>',_Tr('VERSION')))
		INFO('I18N',I18N.LangPack.GetSign())
	end)
	mc.regConsoleCmd('iland language list',_Tr('command.console.land_language_list'),function(args)
	...
	mc.regConsoleCmd('iland language list-online',_Tr('command.console.land_language_list-online'),function(args)
	...
	mc.regConsoleCmd('iland language install',_Tr('command.console.land_language_install'),function(args)
		if not args[1] then
	...
	mc.regConsoleCmd('iland language update',_Tr('command.console.land_language_update'),function(args)
		local list = I18N.LangPack.GetInstalled()
		if not args[1] then
	...
	mc.regConsoleCmd('iland reload',_Tr('command.console.land_reload'),function(args)
		Plugin.Reload()
	end)
	mc.regConsoleCmd('iland unload',_Tr('command.console.land_unload'),function(args)
		Plugin.Unload()
	end)
...
mc.listen('onStartDestroyBlock',function(player,block)
	...
	if MEM[xuid].rsr and (not MEM[xuid].rsr.sleep or MEM[xuid].rsr.sleep<os.time()) then
		-- local HandItem = player:getHand()    -- Edited
...
mc.listen("onOpenContainer", function(pl, bl)
    local landId = Land.Query.Pos(bl.pos)
    if landId == -1 then return end
    if not landId then return end
    if pl.xuid == nil then return end
    if Land.Util.CheckPerm(landId, pl.xuid) then return end
    local perm = DataStorage.Land.Raw[landId].permissions
    if string.find(bl.type, "shulker") and not perm.use_shulker_box then return false end
    if bl.type == "minecraft:chest" and not perm.allow_open_chest then return false end
end)

-- 使用物品时
mc.listen("onUseItemOn", function(pl, item, bl, side, pos)
    local landId = Land.Query.Pos(bl.pos)
    if landId == -1 then return end
    if not landId then return end
    if pl.xuid == nil then return end
    if Land.Util.CheckPerm(landId, pl.xuid) then return end
    if bl.type == "minecraft:grass" then return false end -- 铲除修护
    if string.find(bl.type, "sign") then return false end -- 告示牌修护
    if bl.type == "minecraft:dragon_egg" then return false end -- 龙蛋传送修护
    if string.find(bl.type, "_log") or string.find(bl.type, "_stem") then return false end -- 挖树皮修护
end)