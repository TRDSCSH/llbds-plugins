	...
	Config = {
		...
		Update = function(origin)
			...
			if origin.version < 260 then
				...
				origin.land.bought.three_dimension.calculate = "0"
				origin.land.bought.two_dimension.calculate = "0"
			end
				...
				function(player,result)
					...
					RangeSelector.Create(player,function(player,cube,dimension)
						MEM[xuid].keepingTitle = {
							...
							_Tr('title.selectland.complete2','<a>',cfg.features.selection.tool_name,'<b>','lands ok')
						}
	...
	Push = function(player,pos)
		...
		elseif mem.step == 5 then
			...
			if MEM[xuid].newLand then
				player:runcmd("lands buy")
			elseif MEM[xuid].reselectLand then
				player:runcmd("lands ok")
			end
			return
		end
	end,
...
function RegisterCommands()
	...
	mc.regPlayerCmd('lands',_Tr('command.land'),function(player,args)
		if #args~=0 then
		...
		if landId and Land.RelationShip.Owner.getXuid(landId) == xuid then
			...
			player:sendForm(
				...
				function(player,id)
					if not id then return end
					if id==0 then
						player:runcmd('lands new')
					elseif id==1 then
						player:runcmd('lands gui')
					elseif id==2 then
						player:runcmd('lands tp')
					end
				end
			)
		end
	end)
	mc.regPlayerCmd('lands new',_Tr('command.land_new'),function (player,args)
		local xuid = player.xuid
		...
		elseif not Land.RelationShip.Operator.check(xuid) and #DataStorage.RelationShip.Raw['Owner'][xuid]>=cfg.land.max_lands then
			...
			RangeSelector.Create(player,function(player,cube,dimension)
				MEM[xuid].keepingTitle = {
					_Tr('title.selectland.complete1'),
					_Tr('title.selectland.complete2','<a>',cfg.features.selection.tool_name,'<b>','lands buy')
				}
	...
	mc.regPlayerCmd('lands giveup',_Tr('command.land_giveup'),function (player,args)
		local xuid = player.xuid
		if MEM[xuid].newLand then
	...
	mc.regPlayerCmd('lands gui',_Tr('command.land_gui'),function (player,args)
		OpenGUI.LMgr(player)
	end)
	mc.regPlayerCmd('lands set',_Tr('command.land_set'),function (player,args)
		local xuid = player.xuid
	...
	mc.regPlayerCmd('lands buy',_Tr('command.land_buy'),function (player,args)
		local xuid = player.xuid
	...
	mc.regPlayerCmd('lands ok',_Tr('command.land_ok'),function (player,args)
		local xuid = player.xuid
	...
	mc.regPlayerCmd('lands mgr',_Tr('command.land_mgr'),function (player,args)
		local xuid = player.xuid
	...
	mc.regPlayerCmd('lands mgr selectool',_Tr('command.land_mgr_selectool'),function (player,args)
		local xuid = player.xuid
	...
	mc.regPlayerCmd('lands tp',_Tr('command.land_tp'),function (player,args)
	...
	mc.regPlayerCmd('lands tp set',_Tr('command.land_tp_set'),function (player,args)
		if not cfg.features.landtp then SendText(player,_Tr('talk.feature.disabled'));return end
		local xuid = player.xuid
		local pos = player.blockPos
	...
	mc.regPlayerCmd('lands tp rm',_Tr('command.land_tp_rm'),function (player,args)
		if not cfg.features.landtp then SendText(player,_Tr('talk.feature.disabled'));return end
		local xuid = player.xuid
		local pos = player.blockPos
