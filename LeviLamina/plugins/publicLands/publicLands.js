...
ll.require("plugins/iland-core/iland-core.lua");

// 导入功能
const PosGetLand = lxl.import('ILAPI_PosGetLand'); // 通过坐标查询领地 param: ( Vec4: {x, y, z, dimid}, noAccessCache: bool ) return: landId - string
const GetName = lxl.import('ILAPI_GetName'); // 获取领地昵称 param: ( landId: 领地ID ) return: string
const GetOwner = lxl.import('ILAPI_GetOwner');
const CreateLand = lxl.import('ILAPI_CreateLand');
const DeleteLand = lxl.import('ILAPI_DeleteLand');
const GetPlayerLands = lxl.import('ILAPI_GetPlayerLands');
const UpdatePermission = lxl.import('ILAPI_UpdatePermission'); // 更新领地权限
const UpdateSetting = lxl.import('ILAPI_UpdateSetting');
const SetOwner = lxl.import('ILAPI_SetOwner');
const GetRange = lxl.import('ILAPI_GetRange');
const AddTrust = lxl.import('ILAPI_AddTrust');
const IsLandOperator = ll.import('ILAPI_IsLandOperator');
...
function isPlandOperator(playerName) {
    const configData = new JsonConfigFile(configFilePath);
    const opList = configData.get('op');
